...
/**
 * XMLMetaDataParser handles reading configuration details for elements and
 * product types from their XML policy files.
 * 
 * @author aclark
 * 
 */
public class XMLMetaDataParser {
    ...
    /**
     * The constructor loads a policy file for parsing by either of the two
     * utility methods, getElements or getProductTypes.
     * 
     * @param filename
     *            The name of the XML policy file to parse for configuration
     *            details.
     */
    ...
    /**
     * getElements() will retrieve all of the elements from a policy file for a
     * given dataset collection and instantiate a collection of objects with
     * their details.
     * 
     * @returns Returns a Hashtable<String, CasElement> of all elements found in
     *          the elements.xml policy file. The hashtable maps element names
     *          to their objects.
     */
    ...
    /**
     * getProductTypes() parses a product type policy file to retrieve all
     * available configuration information for a given dataset.
     * 
     * @returns A Hashtable<String, CasProductType> of product type information
     *          read from a policy file. Product type metadata, extractor
     *          configuration, and file management information (e.g. versioner
     *          class, repository path) is stored in each object. Product type
     *          names are mapped to their corresponding objects in the
     *          hashtable.
     * 
     */
    public Hashtable<String, CasProductType> getProductTypes() {
        ...
        for (int i = 0; i < list.getLength(); i++) {
            ...
            // retrieve the product-type file management metadata and
            // description
            ...
            if (sublist.getLength() > 0) {
                ...
                if (extractors.getLength() > 0) {
                    for (int m = 0; m < extractors.getLength(); m++) {
                        ...
                        String extractorKey = tmpExtractor
                                .getAttribute("class");
                        ...
                        if (config.getLength() > 0) {
                            ...
                            NodeList props = configElement
                                    .getElementsByTagName("property");
                            if (props.getLength() > 0) {
                                for (int k = 0; k < props.getLength(); k++) {
                                    Element tmpProperty = (Element) props
                                            .item(k);

                                    String propName = tmpProperty
                                            .getAttribute("name");
                                    String propValue = tmpProperty
                                            .getAttribute("value");
                                    cpt.setConfigurationPropertyValue(
                                            extractorKey, propName, propValue);
                                }
                            }
                        }
                    }
            ...
            if (sublist.getLength() > 0) {
                ...
                NodeList keyValPairs = metadataTag
                        .getElementsByTagName("keyval");
    ...
    /**
     * Parses an XML file and returns a Document object storing that XML
     * structure.
     * 
     * @param filename
     *            The name of the XML policy file to be parsed.
     * 
     * @returns A Document object storing the DOM tree of the parsed XML file.
     */
